...
$tableArray = array();
$tableCustomName = array("Id","Registerno","Coursecode","FirstInternalmark","SecondInternalmark","ThirdInternalmark","FinalInternalmark","AssignmentMark","Total");

// ----------------------------------FIELD INSERTING IN DROP DOWN
  function TableNameListOut()
  {
    global $mysqli,$tableCustomName;
    ...
    if($tableres->num_rows>0){
      ...
      while($tablerow = $tableres->fetch_assoc()){
        ...
        echo "<option value='$tableField'>{$tableCustomName[$index]}</option>";
        $index = $index  + 1;
...
<body>
    <h2>View Student Internal Marks Records</h2>
...
<div class="card">
  <form action="<?php $_SERVER['PHP_SELF']?>" method="post" name='frm12' >
      <select name="tablefield" id="tablefield" class="dropdown">
 ...
 <script type="text/javascript">
  ...
  <form class="form"action="Nominal_update.php" id="form1"method="post" name="frm1">
   <!--  <input type="checkbox" id="flexCheck"class="check"name="Allcheck" value=""></td>
 -->
...
<tr>
<th><strong>Checkboxes</strong><input type="checkbox" id="flexCheck"class="check"name="Allcheck[]"value="<?= $row["id"]; ?>"></td>
</th>
<th><strong>S.No</strong></th>

<th><strong>Reg No</strong></th>
<th><strong>Cource Code</strong></th>
<th><strong>First Internal</strong></th>
<th><strong>Second Internal</strong></th>
<th><strong>Third Internal</strong></th>
...
<tbody>
<?php
$Coursecode = $_SESSION['CourseCode_View'];
echo "<script>alert($Coursecode);</script>";
$tableName = "nominal_tb";
$sel_query="Select * from {$tableName} where tb_Course_code = '{$Coursecode}'";
$result = mysqli_query($mysqli,$sel_query);
while($row = mysqli_fetch_assoc($result)) { 


  $Registerno = $row["tb_Register_no"];
...
// var Check_boxes = document.getElementsByName('Allcheck');
...
// Check_boxes.addEventListener('change',function(){
//   if(this.checked == true){
//       document.getElementById("dataTable").readOnly = false; 
