...
<html>
<head>
	<title>Internal file Uploads</title>
    ...
    <div class="cols-md-6">
        ...
        <form action="Excel_index.php" method="post" enctype="multipart/form-data">
            ...
            <div class="custom-file">
                ...
                <label for="formFileMultiple" class="custom-file-label" for="customFileInput">Select Your Internal File</label>
           ...
           foreach ($Reader as $Row)
            {
                ...
                $Fmark = mysqli_real_escape_string($conn,$Row[2]);
                $Smark = mysqli_real_escape_string($conn,$Row[3]);
                $Tmark = mysqli_real_escape_string($conn,$Row[4]);
                // $final = mysqli_real_escape_string($conn,$Rw[5]);
                $Amark = mysqli_real_escape_string($conn,$Row[5]);
                // $total = mysqli_real_escape_string($conn,$Row[7]);
                ...
                // echo gettype($Smark);
                if (!empty($Regno) || !empty($Ccode) || !empty($Fmark) || !empty($Smark) || !empty($Tmark)|| !empty($Amark)) {

                    
                    $query = "INSERT INTO nominal_tb (tb_Register_no,tb_Course_code,tb_First_int_mark,tb_Second_int_mark,tb_Third_int_mark,tb_int_final_mark,tb_Assign_mark,tb_total_mark )  VALUES ('{$Regno}','{$Ccode}','{$Fmark}','{$Smark}','{$Tmark}','{$Final}','{$Assign}','{$Total_Mark}')";
